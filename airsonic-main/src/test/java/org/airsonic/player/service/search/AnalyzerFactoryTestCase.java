...
public class AnalyzerFactoryTestCase {
  ...
  public void testTokenCounts() {
     ...
     * It is also possible to apply definitions
     * such as "id3 delimiters Tokenizer" to specific fields.
     */
    ...
    Arrays.stream(IndexType.values()).flatMap(i -> Arrays.stream(i.getFields())).forEach(n -> {
      ...
      switch (n) {
        ...
        /*
         * In the legacy, these field divide input into 7.
         * It is not necessary to delimit this field originally.
         */
        ...
        default:
           ...
           * because the input value does not contain a delimiter.
           */
          assertEquals("oneTokenFields : " + n, 7, terms.size());
          break;
      }
    ...
    Arrays.stream(IndexType.values()).flatMap(i -> Arrays.stream(i.getFields())).forEach(n -> {
      ...
      switch (n) {
        ...
        default:
          ...
          /*
           * ID, FOLDER_ID, YEAR This is not a problem
           * because the input value does not contain a delimiter.
           */
          assertEquals("tokenized : " + n, 2, terms.size());
          break;
  ...
  private List<String> toTermString(String field, String str) {
    ...
    try {
      ...
      while (stream.incrementToken()) {
        result.add(stream.getAttribute(TermAttribute.class).toString().replaceAll("^term\\=", ""));
      }
      stream.close();
    } catch (IOException e) {
      LoggerFactory.getLogger(AnalyzerFactoryTestCase.class).error("Error during Token processing.",
          e);
    }
    return result;
  }
